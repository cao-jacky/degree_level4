...
def old_voigt_fitter(cube_id):
    ...
    z_wl_masked = model_wl_masked # redshifted wavelength range
    ...
    voigt = VoigtModel(prefix='v1_') + ConstantModel()
    ...
    sigma_opt_kms = (sigma_opt) / (peaks_av * (1+z)) * speed_of_light
...
def new_voigt_fitter(cube_id):
    ...
    ppxf_spec = np.load("ppxf_results/cube_" + str(int(cube_id)) + "/cube_" + 
            str(int(cube_id)) + "_y.npy")
    ...
    #ppxf_spec_masked = ppxf_spec[model_mask]
    #ax.plot(model_wl_masked, ppxf_spec_masked, lw=1.5, c="#000000", alpha=0.3)
