...
(\textit{This table is adapted from a version from An Introduction to Modern Astrophysics by \cite{carroll_astro}}.)}
...
After a successful fitting, the following variables would be outputted: $c$, $I_1$, $I_2$, $z$, and $\sigma_{\text{gal}}$, where the latter two would be the most useful in later research. 

The redshifts along with the [OII] flux $f_{\text{[OII]}}(x)$ can be used to calculate the luminosity for the doublet, then the [OII] luminosity can be plotted against the redshifts (Figure \ref{fig:oiiluminosity_redshift})

\begin{center}
\includegraphics[width=1.0\linewidth]{data/o_ii_luminosity_vs_redshift}
\captionof{figure}[OII Luminosity vs. Redshift]{[OII] luminosity plotted against the redshift $z$. Variables used for each galaxy were obtained from fitting the galaxy-integrated spectra with the model in Equation \ref{eqn:doublet}. The line represents the lower-limit of the sample as it is using the galaxy with the smallest flux. All galaxies in the set are above this line and approximately follow the luminosity-redshift trend.}
