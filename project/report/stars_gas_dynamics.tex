...
As a synthetic test of pPXF, 10 galaxy spectra were each perturbed 300 times by sequentially adding random Gaussian noise. During each perturbation the spectra was passed to pPXF and all associated outputs were saved and recorded. This allows for the S/N to be plotted against the modulus difference between the optimal line widths and the perturbed ones (Figure \ref{fig:sn_difference}). One finds that as the S/N decreases, the difference (or fractional error) should increase dramatically as there would not be enough resolution for emission or absorption features to be discerned. 
