...
For every doublet fitting performed, the region $3600\times(1+z)$ to $36750\times(1+z)$ was extracted to isolate the peaks from other absorption or emission features. The following equation was then used as the model,
\begin{multline}
f(x) = cx + \frac{I_1}{\sigma \sqrt{2\pi}} \exp{-\frac{(x-L_1)^2}{2\sigma^2}} \\
+ \frac{I_2}{\sigma \sqrt{2\pi}} \exp{-\frac{(x-L_2)^2}{2\sigma^2}},
\end{multline} 

where $c$ is a constant, $\sigma=\sqrt{\sigma_{\text{gal}}^2 + \sigma_{\text{inst}}^2}$, $L_1=3727.092\times(1+z)$, $L_2=3728.875\times(1+z)$

% information on what the doublet actually is, provide both wavelengths of the peaks 

% provide constraints used in the doublet fitting?

% provide a reference for the [O II] feature representing the gas in galaxies

% provide the equation used to add the Gaussian's together
