...
It would be quite non-sensical to use the entire collection of extracted objects as there are more than 1000 of them and the majority of are not high enough in S/N for spectra evaluation. In the analysis performed here, the entire catalogue was sorted by the V-band magnitude and then only the 
...
Through collapsing the entire galaxy into one spectrum, one would find that the most prominent feature is the O[II] doublet emission line. In Figure \ref{fig:ppxf_spectra} it can be seen that it is the largest by amplitude and it is the easiest to model for as it is two Gaussian profiles added together.
