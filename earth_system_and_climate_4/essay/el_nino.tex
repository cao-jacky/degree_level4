...
Persistence is an another analysis of the correlation, however it looks at the fixed-phase correlation between different months within a single time series \citep{troup1965southern}. Contrasting with autocorrelation, which is independent of the starting month, persistence allows you to see any seasonal changes in the correlation between one month and the next. \\

\begin{figure}
\includegraphics[width=\textwidth]{data/persistence_sst_soi}
\captionsetup{justification=justified}
\caption[Persistence]{Taken from \cite{torrence1998annual}. (a) Persistence of NINO3 surface sea temperature (SST) with each curve shifted to line up with the starting month on the top axis (JAJO=January, April, July, October), and the corresponding lag month on the lower axis. }
