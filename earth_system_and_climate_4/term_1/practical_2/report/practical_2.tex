...
One prevailing issue is why do the orbital features in the benthic foram data not span the entirety of the data set? If the hypothesis was to be taken at face value then it would be expected that the signals should be seen across the WT heatmaps (as in Fig.~\ref{fig:wa_orbital_data}). Take obliquity, covering only $\sim 1.13$ Myr yet from REDFIT (Fig.~\ref{fig:d18o_redfit}) it was found to be the strongest signal. This cut-off is attributed to the out of phase precessional phases between the northern and southern hemispheres in the period of $\sim 1.0$ Myr to 3.0 Myr BP. The 23 ka changes in ice volume in the benthic $\delta^{18}$O data in each hemisphere cancel out which leaves the in-phase obliquity to dominate \cite{raymo_plio}. \\
