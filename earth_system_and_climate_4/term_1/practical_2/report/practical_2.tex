...
% do I really need a description of the statistical techniques, I should just describe that they work and that's that
...
One prevailing issue is why do the orbital features in the benthic foram data not span the entirety of the data set? If the hypothesis was to be taken at face value then it would be expected that the signals should be seen across the WT heatmaps (as in Fig.~\ref{fig:wa_orbital_data}). In particular the obliquity only covers $\sim 1.13$ Myr yet from REDFIT (Fig.~\ref{fig:d18o_redfit}) it was found to be the strongest signal. M.~E.~Raymo et al.~modelled how local insolation changes would affect the Antarctic ice sheets from $\sim 1.0$ Myr to 3.0 Myr BP \cite{raymo_plio}. It was found that the dominant 41.1 ka obliquity signal could be the result of out-of-phase ice sheet growth at each pole, because changes in ice volume in each hemisphere would cancel the other out. They further propose that the long-term cooling from the transition of land-based to marine-based East Antarctic Ice Sheet led to a strengthening of the 23 ka cycles at $\sim 2.0$ Myr. Analysis of our data does verify this, as whilst there is not a smooth horizontal signal, there are smaller patches which indicate a continued signal. \\
